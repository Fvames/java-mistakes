...
public class FilesTreamOperationNeedClose {
    ...
    private static void linesTest() throws IOException {
        ...
        log.info("lines {}", Files.lines(Paths.get("large.txt")).limit(500000).collect(Collectors.toList()).size());
        stopWatch.stop();
                ...
                IntStream.rangeClosed(1, 10).forEach(__ -> {
                    try {
                        Files.write(Paths.get("large.txt"),
                                IntStream.rangeClosed(1, 50000).mapToObj(i -> payload).collect(Collectors.toList())
                                , UTF_8, CREATE, APPEND);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
